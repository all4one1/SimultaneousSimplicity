...
namespace stream_cpu
{
	...
	void form_matrix_with_2d_laplace(SparseMatrix& M, Configuration& c, bc_type bcx, bc_type bcy, double coef = 1.0, double tau = 1e+10)
	{
		M.resize(c.N);
	...
	struct ImplicitStream
	{
		...
		{
			form_matrix_with_2d_laplace(SMT, c, c.xbc == periodic ? periodic : Neumann, Dirichlet, (1.0 / c.Pr), c.tau);
			form_matrix_with_2d_laplace(SMC, c, c.xbc == periodic ? periodic : Dirichlet, Dirichlet, 1.0, c.tau);
			form_matrix_with_2d_laplace(SMO, c, c.xbc == periodic ? periodic : Dirichlet, Dirichlet, 1.0, c.tau);
			form_matrix_with_2d_laplace(SMP, c, c.xbc == periodic ? periodic : Dirichlet, Dirichlet, 1.0);
		}
	};
}